...
// Actions are children, spaced by a layout primitive.
...
| --- | --- | --- |
| `'off'` *(default)* | nothing | Anything present when the page renders |
| `'polite'` | `role="status"` | Something that appeared because the user did something |
| `'assertive'` | `role="alert"` | Something that must interrupt |

Mounting an element that already carries `role="alert"` is the *less* reliable
way to announce something; the reliable way is a region that exists first and
receives text afterwards, which is `Toast`'s architecture. For a form error
summary, move focus to the alert instead — `ref` is forwarded and `tabIndex`
passes through.
...
| --- | --- | --- | --- |
| `tone` | `'neutral' \| 'accent' \| 'danger' \| 'success' \| 'warning'` | `'neutral'` | Sets the tone context, so children inherit the hue |
