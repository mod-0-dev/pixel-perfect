'use client';

import { Field, Stack, Switch, Text } from 'pixel-perfect';
import { useState } from 'react';

/**
 * The only interactive island on this page, which is why it is a file of its
 * own rather than a `'use client'` at the top of `page.tsx`. The page stays a
 * Server Component and renders this client component without becoming one.
 *
 * Every id here is written by hand, so it lives OUTSIDE the Matrix (D-035 §1).
 * The harness renders its subtree six times; six copies of `ship-on-merge`
 * would bind five labels to a control in another cell.
 *
 * It is also the demo that shows what a switch IS: the line below changes as
 * you flip it, with no Save button anywhere. A switch whose effect waits for a
 * submit is a checkbox.
 */
export function Controlled() {
  const [shipOnMerge, setShipOnMerge] = useState(false);

  return (
    <Stack gap="2">
      <Field
        label="Ship on merge"
        description="Deploys to production as soon as a PR lands."
        orientation="horizontal"
        controlId="ship-on-merge"
      >
        <Switch checked={shipOnMerge} onCheckedChange={setShipOnMerge} />
      </Field>

      <Text size="sm" tone="muted">
        {shipOnMerge
          ? 'Merging a PR deploys it. Already in effect — nothing to save.'
          : 'Merges do nothing until you deploy by hand.'}
      </Text>
    </Stack>
  );
}
